...
class UltimateDeltaCLI:
    ...
    def solve_captcha_ai(self, instruction, image_path, num_images):
        if not GEMINI_API_KEY: return None
        print("  [*] Transmitting CAPTCHA to Gemini 2.5 Flash API...")
        try:
            with open(image_path, "rb") as f:
                img_data = base64.b64encode(f.read()).decode('utf-8')
            prompt = f"Look at this image grid. The images are labeled with numbers 1 to {num_images} in their corners. The instruction is: '{instruction}'. Find the image that best matches the instruction and reply with ONLY its number as a single integer."
            url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={GEMINI_API_KEY}"
            payload = {"contents": [{"parts": [{"text": prompt}, {"inlineData": {"mimeType": "image/png", "data": img_data}}]}], "generationConfig": {"temperature": 0.0}}
            r = requests.post(url, json=payload, timeout=20)
            if r.status_code == 200:
                answer = r.json()['candidates'][0]['content']['parts'][0]['text'].strip()
                match = re.search(r'\d+', answer)
                if match:
                    ans_num = int(match.group())
                    if 1 <= ans_num <= num_images:
                        print(f"  [+] Gemini solved index: {ans_num}")
                        return str(ans_num)
        except Exception as e: print(f"  [-] Gemini API Error: {e}")
        return None
    ...
    def do_sentry_step(self, sentry_url):
        ...
        if images:
            ...
            print(f"  [+] Grid saved locally as 'captcha_grid.png' ({len(images)} shapes).")

        start_solve_time = time.time()
        
        ai_ans = self.solve_captcha_ai(instruction, "captcha_grid.png", len(images))
        if ai_ans:
            user_input = ai_ans
            time_spent = time.time() - start_solve_time
            if time_spent < 4.5:
                time.sleep(4.5 - time_spent)
        else:
...
if __name__ == '__main__':
    print("="*60)
    print(" DELTA CLI TOOL - FULL AUTO SENTRY (GEMINI) & BYPASS")
    print("="*60)
